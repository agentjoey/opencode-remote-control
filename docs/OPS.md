...
### 模式 B — Standalone host（opencode + ACP 多后端，可界面切换）

独立进程，不作为 opencode 插件，可同时挂多个 agent 并在界面里切换。前置：要用的
ACP agent 已登录（如 `kimi login`）。
...
# 3. 启动（必须在有完整 PATH 的真实终端跑——host 要 spawn kimi/opencode 子进程）
scripts/run-acp-host.sh
```

**验证（模式 B）：**
```bash
curl -s -H "Authorization: Bearer <WEB_TOKEN>" http://127.0.0.1:17085/api/backends
# → {"backends":[{"id":"opencode",...},{"id":"acp:kimi",...}],"activeId":...}
```
浏览器开 `http://127.0.0.1:17085/#token=<WEB_TOKEN>` → 顶栏出现后端切换器。

**常驻自启（生产推荐）** —— 用 launchd（开机自启 + 崩溃自愈）：
```bash
sed "s#__HOME__#$HOME#g" deploy/com.ocrc.host.plist > ~/Library/LaunchAgents/com.ocrc.host.plist
# 按需编辑 plist 里的 EnvironmentVariables（OCRC_BACKENDS / WEB_PORT / WEB_PUBLIC_URL）
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.ocrc.host.plist
launchctl list | grep ocrc        # 确认在线
```
详见下方「Standalone host 模式」+「launchd 自启服务」。

### 公网访问（手机 PWA 需 HTTPS）
`localhost` 仅本机有效。手机装 PWA 需 HTTPS → 挂隧道（指到对应 WEB_PORT）：
| 方式 | 命令 | 说明 |
|---|---|---|
| cloudflared（自有域名） | ingress 配 `service: http://localhost:<port>` | 稳定，需域名（见「把域名指向 host」） |
| Tailscale | `tailscale serve <port>` | 稳定 `*.ts.net`，设备级认证 |
...
- **Token 安全**：`init` 向导把 Telegram token **明文写进 `.env`**（已 gitignore）。
  生产加固：存 macOS Keychain，`.zshrc` 里取——
  ```bash
  security add-generic-password -a "$USER" -s "ocrc-telegram-bot-token" -w '<token>'
  # .zshrc:
  export TELEGRAM_BOT_TOKEN="$(security find-generic-password -a "$USER" -s ocrc-telegram-bot-token -w 2>/dev/null)"
  ```
