...
describe('POST /api/sessions/:id/rename', () => {
  it('updates the session title', async () => {
    ...
    const app = new Hono(); registerRename(app, singleBackendRegistry({ renameSession } as any))
  ...
  it('400s on empty title', async () => {
    const app = new Hono(); registerRename(app, singleBackendRegistry({ renameSession: vi.fn() } as any))
